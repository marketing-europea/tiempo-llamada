...
if vista == "Ayuda y documentación":
    ...
    st.info(
        "Esta pantalla se puede consultar sin subir archivos ni introducir API Key."
    )

    st.markdown("""
    ## ¿Para qué sirve esta app?

    Esta aplicación analiza:

    - La primera gestión realizada tras cada asignación.
    - El primer contacto comercial real.
    - Cambios de propietario.
    - Reasignaciones.
    - Actividades registradas en el flow de Pipedrive.

    La app utiliza el **Flow API de Pipedrive** como fuente de verdad.
    ...
    Permite excluir leads con preferencias de contacto.
    ...
    ## Qué reconstruye la app

    Para cada negocio:

    1. Fecha de creación.
    2. Propietario inicial.
    3. Reasignaciones.
    4. Actividades posteriores.
    5. Primera gestión.
    6. Primer contacto.
    7. Tiempo desde asignación hasta contacto.
    ...
    Solo cuenta llamadas salientes.
    ...
    Elimina contactos tardíos.

    ### Ocultar tramos sin contacto

    Oculta asignaciones sin gestión.

    ### Analizar solo tramos donde el primer evento sea llamada saliente

    Mantiene únicamente leads donde el primer evento relevante fue una llamada.

    ### Excluir leads con preferencias de contacto

    Usa el Excel de notas para excluir esos casos.
...
st.write(
    ...
    "Opcionalmente puedes subir un Excel de notas. "
    "La app usa el Flow API de Pipedrive para reconstruir asignaciones y contactos."
)
...
uploaded = st.file_uploader(
    ...
    type=["xlsx"],
    key="excel_principal_unico"
)

uploaded_notes = st.file_uploader(
    "Sube el Excel de notas (.xlsx) - opcional",
    type=["xlsx"],
    key="excel_notas_unico"
)
...
if uploaded is None:
    st.warning("Sube el Excel principal para continuar.")
    st.stop()

if not api_token:
    st.warning("Introduce el API token.")
    st.stop()

if not company_domain:
    st.warning("Introduce el subdominio de Pipedrive.")
    st.stop()
